...
RSpec.describe Referrer::Source, type: :model do
  ...
  describe 'priority', with_session: true do
    ...
    describe 'active_from' do
      it 'should be generated by default' do
        source = @session.sources.create!(referrer: '', entry_point: 'https://www.dummy.com/welcome',
                                          client_duplicate_id: 1)
        expect(source.active_from.to_i).to be_within(2).of(Time.now.to_i)
      end

      it 'should be custom' do
        source = @session.sources.create!(referrer: '', entry_point: 'https://www.dummy.com/welcome',
                                          client_duplicate_id: 1, active_from: 2.days.ago)
